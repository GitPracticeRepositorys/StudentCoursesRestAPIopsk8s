pipeline {
    ...
    stages {
        ...
        stage('deploy') {
            steps {
                sh "cd deployments/courses/overlays/develop && edit set image courses=shivakrishna/courses:develop-$env.BUILD_ID"
                sh 'kubectl apply -f deployments/courses/overlays/develop'
            }
        }
